'use client';

import Link from 'next/link';
import { useState, useEffect } from 'react';

export default function Footer() {
  const [year, setYear] = useState<number | null>(null);

  useEffect(() => {
    setYear(new Date().getFullYear());
  }, []);

  return (
    <footer className="bg-white dark:bg-black border-t border-gray-200 dark:border-gray-800 mt-auto" suppressHydrationWarning>
      <div className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8" suppressHydrationWarning>
        <div className="flex flex-col md:flex-row justify-between items-center" suppressHydrationWarning>
          <div className="text-sm text-gray-500 dark:text-gray-400" suppressHydrationWarning>
            © {year ?? '2025'} FastGeo. All rights reserved.
          </div>
          <div className="flex space-x-6 mt-4 md:mt-0" suppressHydrationWarning>
            <Link href="/privacy" className="text-sm text-gray-500 hover:text-gray-900 dark:hover:text-gray-300">
              Privacy Policy
            </Link>
            <Link href="#" className="text-sm text-gray-500 hover:text-gray-900 dark:hover:text-gray-300">
              FAQ
            </Link>
          </div>
        </div>
      </div>
    </footer>
  );
}
